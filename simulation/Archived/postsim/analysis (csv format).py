...
def titles(ax, code = 0):
    # give mpl plots the labels and titles fitting the code number
    if (code & 0b10): # cylindrical
        ax.set_xlabel('Z')
        ax.set_ylabel('R')
    else:
        ax.set_xlabel('X')
...
if __name__ == '__main__':
    ...
    """ Now use Spyder for analysis: edit the filename directly
    if len(sys.argv)==1:
    ...
    df = pd.read_csv(filename+'.csv', usecols=[1,2,3], dtype=float) # Reading rows. 
    ...
    if (code & 2):
        ...
        if (code & 0b10): # cylindrical
            plt.xlabel('R/m')
            plt.ylabel('Z/m')
            plt.hist2d(y, x, bins=50, cmap='Wistia', norm = mpl.colors.LogNorm())
        else:
            plt.xlabel('X/m')
            plt.ylabel('Y/m')
            plt.hist2d(x, y, bins=50, cmap='Wistia', norm = mpl.colors.LogNorm())
        cb = plt.colorbar()
        ...
        if (code & 0b10): # cylindrical
            plt.xlabel('φ/deg')
            plt.ylabel('Z/m')
            plt.hist2d(z, x, bins=50, cmap='Wistia', norm = mpl.colors.LogNorm())
        else:
            plt.xlabel('X/m')
            plt.ylabel('Z/m')
            plt.hist2d(x, z, bins=50, cmap='Wistia', norm = mpl.colors.LogNorm())
        cb = plt.colorbar()
        ...
        if (code & 0b10): # cylindrical
            plt.xlabel('φ/deg')
            plt.ylabel('R/m')
            plt.hist2d(z, y, bins=50, cmap='Wistia', norm = mpl.colors.LogNorm())
        else:
            plt.xlabel('Y/m')
            plt.ylabel('Z/m')
            plt.hist2d(y, z, bins=50, cmap='Wistia', norm = mpl.colors.LogNorm())
        cb = plt.colorbar()
        ...
        if (code & 0b10): # cylindrical
            ...
            ax.set_xlabel('X/m')
            ax.set_ylabel('Y/m')
        fig.scatter(x,y, s=1, alpha=0.01)
        fig.savefig(filename + '_xy_scatter.png')
        fig.cla()
        ...
        if (code & 0b10): # cylindrical
            ...
            ax.set_xlabel('X/m')
            ax.set_ylabel('Z/m')
        fig.scatter(x,z, s=1, alpha=0.01)
        fig.savefig(filename + '_xz_scatter.png')
        fig.cla()
        ...
        if (code & 0b10): # cylindrical
            ...
            ax.set_xlabel('Y/m')
            ax.set_ylabel('Z/m')
        fig.scatter(y,z, s=1, alpha=0.01)
        fig.savefig(filename + '_yz_scatter.png')
        fig.cla()
